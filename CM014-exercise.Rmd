...
For example it would look like y = a1x1 + a2x2 + ... + intersect
...
```{r}
(my_lm <- lm(formula = lifeExp ~ year, gapminder_France))
```
...
```{r}
(my_lm <- lm(formula = lifeExp ~ I(year-1952), data = gapminder_France))
```

```{r}
my_resid <- augment(my_lm) #To get the residuals dataframe
my_resid

ggplot(my_resid, aes(.resid)) +
  geom_freqpoly(binwidth = 0.5) #Plotting the residuals
```
...
```{r}
gapminder_France2000 <- data.frame(year = seq(2000,2005))
predict(my_lm, newdata = gapminder_France2000) %>% 
  head()
```
...
```{r}
ggplot(gapminder_Zimbabwe, aes(year,lifeExp)) + geom_point()+geom_smooth(method = "lm", se = F) #se shows standard errors
```
