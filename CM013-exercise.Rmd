...
1. Clearly labelled
1. Easy to understand/not overly complicated
...
- Weak: we don't know the standard deviation; the difference between countries is exaggerated;we need to consider where the axis starts. motivation is inferred from number of hours worked, we also do not know the sample size. 
...
- Do not know. The years are not labelled. The y-axis is too broad. We need to know that a 2 degree difference is actually significant.
...
- Very weak. The gun deaths are actually increasing. The y-axis is flipped, so the graph is very counterintuitive.
...
There is a factor of 10 between both y axes, so the lines are actually not tracking.
...
t <- gapminder %>%
  plot_ly(x = ~gdpPercap, 
          ...
          color = ~continent, 
          type = 'scatter',
          mode = 'markers') #if you want a line graph, say line instead fo markers

sys.setenv("plotly_username" = "")
sys.setenv("plotly_api_key" = "")

api_create(t, filename = 'cm013-plotly=example') #You can zoom in and out and share it with other people to view
```
